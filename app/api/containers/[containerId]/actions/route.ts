...
import { type ContainerAction, isSystemContainer } from '@/lib/container-inventory';
import { runContainerAction } from '@/lib/container-runtime';
...
export async function POST(
  ...
  try {
    ...
    const runtime =
      snapshot.containers.all.find((container) => container.id === containerId) ?? null;
    ...
    if (isSystemContainer(runtime) && body.action !== 'restart') {
      throw new Error('Protected system containers only allow restart from this page.');
    }
