...
export async function POST(
  ...
  try {
    ...
    const runtime = snapshot.containers.all.find((c) => c.id === containerId) ?? null;
    ...
    const changes: RecreateChanges = {
      envVars: Array.isArray(body.envVars)
        ...
        : undefined,
      exposureMode: isExposureMode(body.exposureMode) ? body.exposureMode : undefined,
      image: typeof body.image === 'string' ? body.image : undefined,
      name: typeof body.name === 'string' ? body.name : undefined,
      port: typeof body.port === 'number' && body.port > 0 ? body.port : undefined,
    };
