...
function getShell() {
  return process.env.SHELL && path.isAbsolute(process.env.SHELL) ? process.env.SHELL : '/bin/bash';
}
...
function appendOutput(current: string, chunk: Buffer, onClip: () => void) {
...
async function resolveWorkingDirectory(value: unknown) {
  const requested =
    typeof value === 'string' && value.trim().length > 0 ? value.trim() : process.cwd();
