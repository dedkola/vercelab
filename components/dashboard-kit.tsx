...
function getIconContent(name: IconName) {
  switch (name) {
    case 'bars':
      return <path d="M6 18V14M10 18V9M14 18V11M18 18V7" {...STROKE_PROPS} strokeWidth={2} />;
    case 'chevron-down':
    ...
    case 'cloud':
      return (
        <path d="M6 18h11a4 4 0 0 0 .5-7.97A7 7 0 0 0 5.2 14 3 3 0 0 0 6 18z" {...STROKE_PROPS} />
      );
    case 'github':
      return (
        <>
          <path
            ...
            {...STROKE_PROPS}
          />
          <path d="M9 18c0-1.7-.1-2.7-1.1-3.3-2.5.3-3.1-1.2-3.3-1.9" {...STROKE_PROPS} />
          <path d="M15 18c0-1.7.1-2.7 1.1-3.3 2.5.3 3.1-1.2 3.3-1.9" {...STROKE_PROPS} />
...
export function Icon({ name, className = 'h-4 w-4', title }: IconProps) {
  ...
  if (title) {
    return (
      <svg aria-label={title} className={className} role="img" viewBox="0 0 24 24">
        <title>{title}</title>
        {content}
      </svg>
    );
