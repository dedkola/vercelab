...
export function ContainersMainContent({
  ...
  return (
    <WorkspaceDialog
      ...
      title={runtimeEntry.sidebarName}
      variant="drawer"
    >
      ...
      <div
        role="tabpanel"
        id="container-management-panel"
        aria-labelledby={`container-management-${activeTab}`}
        tabIndex={0}
        className="min-h-0 min-w-0 flex-1 overflow-y-auto [overflow-wrap:anywhere] bg-[var(--canvas)] p-4 max-[640px]:p-3"
      >
        {inspectError ? <WorkspaceNotice className="mb-3">{inspectError}</WorkspaceNotice> : null}
        {activeTab === 'overview' ? (
          <div className="space-y-4">
            ...
            {actionError ? <WorkspaceNotice>{actionError}</WorkspaceNotice> : null}
            ...
            {inventoryMeta.kind === 'system' ? (
              ...
              <>
                ...
                >
                  ...
                  {tagsError ? <WorkspaceNotice>{tagsError}</WorkspaceNotice> : null}
                ...
                <div className="grid grid-cols-2 gap-3 max-[520px]:grid-cols-1">
                  ...
                  >
                    <select
                      ...
                      onChange={(event) => setEditExposureMode(event.target.value as ExposureMode)}
        ...
        {activeTab === 'variables' ? (
          <div className="space-y-3">
            ...
            {inventoryMeta.kind === 'system' ? (
              ...
              <fieldset disabled={inspectLoading || !inspectData}>
        ...
        {activeTab === 'logs' ? (
          <div className="space-y-3">
            ...
            <section className="overflow-hidden rounded-[10px] border border-[var(--hairline)] bg-[var(--surface-subtle)]">
              <header className="flex items-center justify-between border-b border-[var(--hairline)] px-3 py-2.5">
                <div className="flex items-center gap-2 text-[11px] text-[var(--muted-ink)]">
              ...
              <div className="min-h-80 space-y-1.5 overflow-x-auto p-3 font-mono text-[11px] leading-5 text-[var(--ink)]">
                {logs.length ? (
                  logs.map((line) => (
                    <div className="grid grid-cols-[auto_auto_minmax(0,1fr)] gap-2" key={line.id}>
                      ...
                      />
                      <span className="whitespace-nowrap text-[var(--quiet)]">
                        {line.timestamp}
                      </span>
                      <span className="[overflow-wrap:anywhere] text-[var(--ink)]">
                        {line.message}
                      </span>
                    </div>
                  ))
                ) : (
                  <p className="text-[var(--quiet)]">No lines in this log view.</p>
                )}
              </div>
            </section>
          </div>
        ...
        <footer className="flex min-h-[54px] shrink-0 flex-wrap items-center justify-between gap-3 border-t border-[var(--hairline)] bg-white px-4 py-3">
          <span className="font-mono text-[10px] text-[var(--quiet)]">
            {recreateError ? (
              ...
              'Changes apply by replacing the current container'
            )}
          </span>
          <Button
            disabled={recreatePending || inspectLoading || !inspectData}
