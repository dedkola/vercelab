...
export async function loadWorkspaceShellData(
  ...
  const influxPromise: Promise<[MetricsHistoryPoint[], ContainerMetricsHistoryPoint[]]> =
    includeMetricsHistory
      ...
      : Promise.resolve([[], []] as [MetricsHistoryPoint[], ContainerMetricsHistoryPoint[]]);

  const [initialSnapshot, initialDeployments, [initialHistory, initialContainerHistory]] =
