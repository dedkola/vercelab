...
>(data: T, ctx: z.RefinementCtx) {
  if ((data.exposureMode === 'tcp' || data.exposureMode === 'host') && !data.hostPort) {
    ctx.addIssue({
...
export const createDeploymentSchema = z
  .object({
    ...
    branch: z.preprocess(normalizeOptionalString, z.string().min(1).max(120).optional()),
    ...
    appName: z.string().trim().min(2, 'App name is too short.').max(60, 'App name is too long.'),
    subdomain: z
      .string()
      .trim()
      .toLowerCase()
    ...
    envVariables: z.preprocess(
      normalizeOptionalString,
      z.string().max(12000, 'Environment variables payload is too large.').optional()
    ),
  })
...
export const updateDeploymentSettingsSchema = deploymentActionSchema
  .extend({
    ...
    commitSha: z.preprocess(
      normalizeOptionalString,
      z.string().regex(commitShaPattern, 'Commit must be a valid git SHA.').optional()
    ),
    subdomain: z
      .string()
      .trim()
