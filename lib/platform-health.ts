...
export async function getPlatformHealth(): Promise<PlatformHealth> {
  ...
  const runtimeSeverity: PlatformCheck['severity'] = strictRuntime ? 'error' : 'warning';
  ...
  if (config.paths.hostRoot) {
    const managedPaths = [config.paths.appsDir, config.paths.logsDir, config.paths.locksDir];
  ...
  const dockerServer = await runProbe('docker', ['version', '--format', '{{.Server.Version}}']);
