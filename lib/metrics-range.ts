...
export function getDashboardHistorySettings(range: DashboardRange, maxPoints = 240) {
  const rangeSeconds = getDashboardRangeSeconds(range);
  const bucketSeconds = Math.max(5, Math.ceil(rangeSeconds / maxPoints / 5) * 5);
  const limit = Math.max(12, Math.min(maxPoints, Math.ceil(rangeSeconds / bucketSeconds)));
