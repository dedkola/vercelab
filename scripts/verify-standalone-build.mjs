...
const standaloneServerRoot = path.join(repositoryRoot, '.next', 'standalone', '.next', 'server');
...
const relativeRequirePattern = /\brequire\(\s*["'](\.[^"']+)["']\s*\)/g;
...
async function verifyStandaloneBuild() {
  ...
  const serverJavaScriptFiles = serverFiles.filter((file) => file.endsWith('.js'));
  ...
  if (missingReferences.length > 0) {
    ...
    throw new Error(`Standalone output contains unresolved relative imports:\n${details}`);
  }
...
verifyStandaloneBuild().catch((error) => {
  console.error(`[standalone-check] ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
