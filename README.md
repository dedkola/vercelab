...
Two modes are available. Pick one — they are fully isolated and can run simultaneously without port or network conflicts.

### Option A — host macOS (recommended for day-to-day work)

Infrastructure (Postgres, InfluxDB, Traefik) runs in Docker. The Next.js app runs directly on your Mac via `pnpm dev`. No system paths are touched — all data lives under `./data/` in the repo root.

**1. Generate `.env.local`** (interactive, one-time):

```bash
pnpm run setup-env
```

The script auto-detects your Mac's LAN IP and Docker socket path, generates a random encryption secret, and backs up any existing `.env.local` before overwriting.
...
| InfluxDB Explorer | `http://influx.localhost` (via Traefik) |
...
Data is persisted in `./data/` (gitignored) between restarts.

---

### Option B — VS Code devcontainer

Everything (Postgres, InfluxDB, Explorer, the Node environment) runs inside Docker. Open the repo in VS Code and choose **Reopen in Container** when prompted, or run **Dev Containers: Reopen in Container** from the command palette.

The devcontainer uses its own isolated Docker network (`vercelab_devcontainer_net`) and named volumes. It does **not** conflict with the host macOS stack.
...
Local development (host macOS, `docker-compose.dev.yml`) stores everything under `./data/` in the repo root — no system paths required:
...
- `./data/traefik/certs`
- `./data/apps`, `./data/logs`, `./data/locks`

The devcontainer uses named Docker volumes instead of host-bind mounts.

Local dev also starts InfluxDB Explorer via Traefik at `http://influx.localhost`.
...
| `VERCELAB_INFLUXDB_EXPLORER_DATA_DIR`       | `${VERCELAB_DATA_ROOT}/influxdb-explorer`                                    | InfluxDB 3 Explorer SQLite data directory                                                                                    |
...
| `VERCELAB_INFLUXDB_EXPLORER_HOST`           | `influx.${VERCELAB_BASE_DOMAIN}`                                             | Public hostname routed to InfluxDB Explorer through Traefik                                                                  |
| `VERCELAB_INFLUXDB_EXPLORER_URL`            | `https://${VERCELAB_INFLUXDB_EXPLORER_HOST}`                                 | Canonical Explorer URL surfaced in the Vercelab UI                                                                           |
...
The Vercelab UI runs inside the `control-plane` service in the root `docker-compose.yml`, with container name `vercelab-ui`.
Core stack container names are now pinned to:
...
- `docker-compose.yml`: production stack — Traefik, Postgres, InfluxDB, and the control plane
- `docker-compose.dev.yml`: local macOS dev infra stack — Traefik (HTTP), Postgres, InfluxDB (no control-plane container)
- `.devcontainer/docker-compose.yml`: VS Code devcontainer stack — fully self-contained, isolated network
- `scripts/setup-env.sh`: interactive `.env.local` generator for local macOS dev
